...
st.markdown("""
    ...
    /* 2. CLEAN TABS (No Boxes, Just Underline) */
    ...
    /* 3. FEATURE CARDS (Adaptive Light/Dark Theme) */
    .feature-card {
        padding: 30px;
        border-radius: 15px;
        text-align: center;
...
def generate_pdf(student_name, prediction, input_data, advice):
    ...
    # 1. SETUP MARGINS & WIDTH (The Fix)
    pdf.set_left_margin(10)
    pdf.set_right_margin(10)
    # A4 width is 210mm. 210 - 10(left) - 10(right) = 190mm printable space
    printable_width = 190
    ...
    for item in advice:
        ...
        # Encode to latin-1 to strip emojis
        clean_text = clean_text.encode('latin-1', 'ignore').decode('latin-1')

        # FIX: Use explicit 'printable_width' instead of 0
...
# --- NEW: RADAR CHART (IDEA 2) ---
def plot_radar_chart(input_values):
    categories = ['Study', 'Sleep', 'Social', 'Exercise', 'Attention']
    # Normalize values roughly to 0-1 scale for chart aesthetics
    # Max assumptions: Study=12, Sleep=10, Social=10, Ex=2, Att=10
    max_vals = [12, 10, 10, 2, 10]
    norm_vals = [v / m for v, m in zip(input_values, max_vals)]

    # Ideal Student Profile (roughly)
    ...
    with col2:
        ...
        if st.session_state['auth_mode'] == 'login':
            with container:
                ...
                st.markdown("---")
                if st.button("New User? Register Here", type="tertiary"):
                    toggle_auth_mode()
                    st.rerun()
        else:  # Register
            with container:
                ...
                with st.form("reg_form"):
                    ...
                    if st.form_submit_button("Register", use_container_width=True):
                        ...
                        if new_user in users:
                            ...
                            save_new_user(new_user, new_email, new_pw)
...
# --- PART B: DASHBOARD ---
    ...
    with tab1:
        ...
        st.markdown("""
        * **Register & Login:** Create your secure account.
        * **Explore Insights:** Use visualization to understand habit impacts.
        * **Train Model:** Upload your class data to learn patterns.
        * **Predict (Individual/Group):** Forecast grades for one student or a whole class.
        * **Get Strategy:** Use the 'What-If' tool to find ways to improve.
        """)
    ...
    with tab2:
        ...
        if train_file:
            try:
                ...
                df_train.columns = df_train.columns.str.strip()  # Fix Column Names
        ...
        if mode == "👤 Individual Prediction":
            ...
            if batch_file and model:
                ...
                try:
                    ...
                    # Add Results
                    df_batch['Predicted_Score'] = [max(0, min(100, p)) for p in preds]


                    # IDEA 3: At-Risk Flags
                    def get_status(s):
                        if s < 50:
                            return "🔴 Critical"
                        elif s < 75:
                    ...
                    df_batch['Status'] = df_batch['Predicted_Score'].apply(get_status)

                    st.dataframe(df_batch)

                    # Summary Stats
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Class Average", f"{df_batch['Predicted_Score'].mean():.2f}")
                    c2.metric("At-Risk Students", len(df_batch[df_batch['Status'] == "🔴 Critical"]))
